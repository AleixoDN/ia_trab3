...
# Função que efetivamente roda o algoritmo de BUSCA BEST-FIRST
...
def find_path_3(init, end, map, map_size):
    ...
    weghts = [sum(map_size)+1]  # Lista de pesos dos pontos salvos na agenda
    ...
    while map[paths[0][-1][0]][paths[0][-1][1]] != '$': # Roda até achar a casa objetivo
        ...
        a = lin_verif_3(paths[0][-1], map, map_size, -1, 0, end) # Verificação para cima
        ...
        a = lin_verif_3(paths[0][-1], map, map_size, 0, 1, end)  # Verificação para a direita
        if a != None:                           # Se a resposta é válida
            agenda_aux.append([a[0], a[1]])     # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append(a[2])             # Salva o peso do ponto em uma lista de pesos auxiliar

        a = lin_verif_3(paths[0][-1], map, map_size, 1, 0, end)  # Verificação para baixo
        if a != None:                           # Se a resposta é válida
            agenda_aux.append([a[0], a[1]])     # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append(a[2])             # Salva o peso do ponto em uma lista de pesos auxiliar

        a = lin_verif_3(paths[0][-1], map, map_size, 0, -1, end) # Verificação para a esquerda
        if a != None:                           # Se a resposta é válida
            agenda_aux.append([a[0], a[1]])     # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append(a[2])             # Salva o peso do ponto em uma lista de pesos auxiliar
        ...
        for i in range(len(agenda_aux)):
            ...
            if repeated == 0:   # Se o ponto não é repetido
                ...
                # de acordo com a diretriz de busca best-first (menor para o maior peso).
                for j in range(len(weghts)):                # Varrendo a lista de pesos
                    if weghts_aux[i] < weghts[j]:           # Se o peso do ponto analisado é menor
