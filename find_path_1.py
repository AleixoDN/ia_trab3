...
# Função que efetivamente roda o algoritmo de BUSCA EM PROFUNDIDADE
def find_path_1(init, map, map_size):
    ...
    while map[paths[0][-1][0]][paths[0][-1][1]] != '$': # Roda até achar a casa objetivo
        ...
        a = lin_verif_1(paths[0][-1], map, map_size, -1, 0) # Varredura para cima
        if a != None:               # Se a resposta é válida
            agenda_aux.append(a)    # Salva o ponto do grafo em uma agenda auxiliar

        a = lin_verif_1(paths[0][-1], map, map_size, 0, 1)  # Varredura para a direita
        if a != None:               # Se a resposta é válida
            agenda_aux.append(a)    # Salva o ponto do grafo em uma agenda auxiliar

        a = lin_verif_1(paths[0][-1], map, map_size, 1, 0)  # Varredura para baixo
        if a != None:               # Se a resposta é válida
            agenda_aux.append(a)    # Salva o ponto do grafo em uma agenda auxiliar

        a = lin_verif_1(paths[0][-1], map, map_size, 0, -1) # Varredura para a esquerda
        if a != None:               # Se a resposta é válida
            agenda_aux.append(a)    # Salva o ponto do grafo em uma agenda auxiliar
        ...
        for i in range(len(agenda_aux)):
            ...
            if repeated == 0:   # Se o ponto não é repetido
                ...
                # de acordo com a diretriz de busca em profundidade (adicionar no inicio da agenda).
                # Por isso insert(0, [])
