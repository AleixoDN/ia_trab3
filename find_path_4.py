...
# Função que efetivamente roda o algoritmo de BUSCA A*
# Para calculo dos pesos, a função heuristica h escolhida foi a Distancia de Manhattan
# A função g é a quantidade de pontos percorridos até o ponto atual
def find_path_4(init, end, map, map_size):
    ...
    weghts = [[0, sum(map_size)+1]] # Lista de pesos dos pontos salvos na agenda
    ...
    while map[paths[0][-1][0]][paths[0][-1][1]] != '$': # Roda até achar a casa objetivo
        ...
        a = lin_verif_4(paths[0][-1], g_now, map, map_size, -1, 0, end) # Verificação para cima
        if a != None:                       # Se a resposta é válida
            agenda_aux.append([a[0], a[1]]) # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append([a[2], a[3]]) # Salva o peso do ponto em uma lista de pesos auxiliar

        a = lin_verif_4(paths[0][-1], g_now, map, map_size, 0, 1, end)  # Verificação para a direita
        if a != None:                       # Se a resposta é válida
            agenda_aux.append([a[0], a[1]]) # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append([a[2], a[3]]) # Salva o peso do ponto em uma lista de pesos auxiliar

        a = lin_verif_4(paths[0][-1], g_now, map, map_size, 1, 0, end)  # Verificação para baixo
        if a != None:                       # Se a resposta é válida
            agenda_aux.append([a[0], a[1]]) # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append([a[2], a[3]]) # Salva o peso do ponto em uma lista de pesos auxiliar

        a = lin_verif_4(paths[0][-1], g_now, map, map_size, 0, -1, end) # Verificação para a esquerda
        if a != None:                       # Se a resposta é válida
            agenda_aux.append([a[0], a[1]]) # Salva o ponto do grafo em uma agenda auxiliar
            weghts_aux.append([a[2], a[3]]) # Salva o peso do ponto em uma lista de pesos auxiliar
        ...
        for i in range(len(agenda_aux)):
            ...
            if repeated == 0:   # Se o ponto não é repetido
                ...
                # de acordo com a diretriz de busca A* (menor para o maior peso).
                for j in range(len(weghts)):                # Varrendo a lista de pesos
                    if sum(weghts_aux[i]) < sum(weghts[j]): # Se a soma dos pesos do ponto analisado é menor
        ...
        g_now = weghts[0][0]    # Adiciona o custo de percurso  do ítem da agenda para o custo de ponto analizado
